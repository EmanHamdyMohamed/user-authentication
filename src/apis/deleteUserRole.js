...
/**
* Delete Role to registered user
*
* @param {any} req The request object
* @param {any} res The response object
* @param {Function} next The callback function
*/
export default async function deleteUserRole(req, res, next) {
    try {
        ...
        if (_.isEmpty(roles)) {
            return res.status(400).send('User roles can not be empty');
        }
        const updatedUser = await Users.updateOne({ email }, { roles });
